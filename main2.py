...
def main(source_video_path: str,
    ...
    for frame in frames_generator:  # 提取影格
        ...
        # 繪製軌跡並計算越線物體的數量
        for box, track_id, track_cls in zip(boxes, track_ids, track_cls):
            ...
            if light_type == "red" and START.x < x < END.x and abs(y - START.y) < 5:  # 假設物體水平交叉
                ...
                if track_id not in car_crossed:
                    if track_cls == 1:  # 0:bus 1:car 2:moto
                ...
                if track_id not in moto_crossed:
                    if track_cls == 2:  # 0:bus 1:car 2:moto
        ...
        count_text = f"car crossed: {len(car_crossed)}"
