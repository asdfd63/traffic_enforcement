...
def main(source_video_path: str,
    ...
    cnt = 1
    for idx, cls in enumerate(types):
        if cls == 3:  # 假設在配置中類別為3的是 area1 下一個是 area2
            if cnt % 2:
    ...
    for frame in frames_generator:  # 提取影格
        ...
        # 繪製軌跡並計算越線物體的數量
        for box, track_id, track_cls in zip(boxes, track_ids, track_clss):
            ...
            # cv2.putText(annotated_frame, f'ID:{track_id}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
            ...
            if track_id in wup and area2 is not None:
                ...
                if result1 >= 0:
                    ...
                    cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (255, 0, 255), 2)
                    cv2.putText(annotated_frame, f'ID:{track_id}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
                    if track_id not in wrongway:
                        wrongway.append(track_id)

            # 檢查物體是否越線（原始功能）
            if track_id in d_dict1:
                d_dict2[track_id] = d_dict1[track_id]
            d = find_point_side(START.x, START.y, END.x, END.y, cx, cy)
            d_dict1[track_id] = d
            if track_id in d_dict2 and d != d_dict2[track_id] and (START.x < cx < END.x or START.x > cx > END.x):
            # 紅燈時檢查物體是否越線（原始功能）
            # if light_type == "red" and START.x < cx < END.x and abs(cy - START.y) < 5:
                # 當物件越過線時對其進行註釋
                cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (0, 255, 0), 1)

                if track_id not in car_crossed and track_cls == 1:  # 假設類別1是汽車
                    car_crossed[track_id] = True
                    if img_cnt < 0:  # 截圖
                ...
                if track_id not in moto_crossed and track_cls == 2:  # 假設類別2是摩托車
                    moto_crossed[track_id] = True
                    if img_cnt < 0:  # 截圖
        ...
        # 顯示統計資訊
        w = len(wrongway)
        count_text = f"Wrong-way Cars: {w}"
        cv2.putText(annotated_frame, count_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        count_text_car = f"Car crossed: {len(car_crossed)}"
        count_text_moto = f"Moto crossed: {len(moto_crossed)}"
        cv2.putText(annotated_frame, count_text_car, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(annotated_frame, count_text_moto, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
